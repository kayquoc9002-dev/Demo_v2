...
import {
  ...
  MOCK_PO,
  type DonDatHang,
} from "../../../components/Kho/data/inboundMockData";
import {
  MOCK_SKU_RULES,
  MOCK_NODES,
...
function ModalTaoPhieu({
  ...
  const available_pos = useMemo(
    () => MOCK_PO.filter((po) => po.trang_thai === "cho_giao"),
    [],
  );
          ...
          >
            ...
            <div className="relative">
              ...
              {ncc_open && (
                ...
                >
                  {MOCK_NHA_CUNG_CAP.filter((n) =>
                    n.toLowerCase().includes(ncc.toLowerCase()),
                  ).map((n) => (
                    <button
                      key={n}
...
function PanelPutAway({
  ...
  const [items, setItems] = useState<PutAwayItem[]>(() =>
    tao_put_away_items(phieu, MOCK_SKU_RULES),
  );
  ...
  const leaf_nodes = useMemo(
    () =>
      MOCK_NODES.filter(
        (n) => la_leaf_node(n.id, MOCK_NODES) && n.trang_thai === "active",
      ),
    [],
  );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        {items.map((item) => {
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          const goi_y_node = MOCK_NODES.find((n) => n.id === item.node_goi_y);
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          const chon_node = MOCK_NODES.find((n) => n.id === item.node_chon);
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          const levels = MOCK_WAREHOUSE_CONFIG.levels;
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p
                      ...
                      style={{ color: "#38bdf8" }}
                    >
                      {tinh_location_code(item.node_goi_y, MOCK_NODES)}
                    </p>
                  </div>
              ...
              {item.da_xac_nhan && item.node_chon && (
                ...
                >
                  ...
                  <code className="font-black">
                    {tinh_location_code(item.node_chon, MOCK_NODES)}
                  </code>
                </div>
              )}
            </div>
...
export default function InboundManager() {
  const [phieu_list, setPhieuList] = useState<PhieuNhapKho[]>(MOCK_PHIEU_NHAP);
                        ...
                        {ct.da_cat_ke ? (
                          <div className="flex items-center gap-1">
                            ...
                            <code
                              ...
                              style={{ color: "#a78bfa" }}
                            >
                              {lay_location_text(ct.node_id_cat, MOCK_NODES)}
                            </code>
                          </div>
                        ...
                        >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setNgayNhanInput(e.target.value)
                              }
                              className="w-full px-3 py-2.5 rounded-xl text-sm outline-none"
                              style={{
                                background: "#1e293b",
...
//   onClose,
// }: {
//   loai_mac_dinh: LoaiPhieuNhap;
//   onSave: (phieu: PhieuNhapKho) => void;
//   onClose: () => void;
// }) {
//   const loai = loai_mac_dinh;

//   // ── Luồng 1: Có PO ──
//   const [selected_po_id, setSelectedPoId] = useState<string>("");
//   const [ngay_nhan, setNgayNhan] = useState("");
//   const [ghi_chu_po, setGhiChuPo] = useState("");
//   const [po_search, setPoSearch] = useState("");

//   // ── Luồng 2: Không PO / Nhập khác ──
//   const [ncc, setNcc] = useState("");
//   const [ncc_open, setNccOpen] = useState(false);
//   const [ngay, setNgay] = useState("");
//   const [ly_do, setLyDo] = useState("");
//   const [ghi_chu, setGhiChu] = useState("");
//   const [rows, setRows] = useState([{ ma_sku: "", ten_sp: "", so_luong: 1 }]);
//   const [sku_open, setSkuOpen] = useState<number | null>(null);
//   const [sku_search, setSkuSearch] = useState("");

//   const [loi_list, setLoiList] = useState<string[]>([]);

//   const po_list_kha_dung = MOCK_PO.filter((p) => p.trang_thai === "cho_giao");

//   const filtered_po = useMemo(() => {
//     const q = po_search.toLowerCase();
//     return q
//       ? po_list_kha_dung.filter(
//           (p) =>
//             p.ma_po.toLowerCase().includes(q) ||
//             p.nha_cung_cap.toLowerCase().includes(q),
//         )
//       : po_list_kha_dung;
//   }, [po_search]);

//   const selected_po = MOCK_PO.find((p) => p.id === selected_po_id);
...
//     if (loai === "tu_don_dat_hang") {
//       if (!selected_po_id) loi.push("Phải chọn một đơn đặt hàng (PO)");
//       if (!ngay_nhan) loi.push("Phải chọn ngày dự kiến nhận hàng");
//     } else {
//       if (!ncc.trim()) loi.push("Nhà cung cấp không được để trống");
//       if (!ngay) loi.push("Phải chọn ngày dự kiến nhận hàng");
//       if (loai === "nhap_khac" && !ly_do.trim())
//         loi.push("Nhập khác bắt buộc phải có lý do");
//       if (rows.every((r) => !r.ma_sku)) loi.push("Phải chọn ít nhất 1 SKU");
//     }
...
//     if (loai === "tu_don_dat_hang" && selected_po) {
//       // Lấy toàn bộ thông tin từ PO — không nhập tay
//       phieu = {
//         id: gen_id(),
//         ma_phieu: gen_ma_phieu(),
//         loai,
...
//         nguoi_tao: "Người dùng hiện tại",
//         ngay_tao: new Date().toISOString(),
//         ngay_du_kien: new Date(ngay_nhan).toISOString(),
//         ngay_nhan_thuc: "",
//         ly_do: "",
//         ghi_chu: ghi_chu_po || selected_po.ghi_chu,
//         chi_tiet: selected_po.chi_tiet.map((ct) => ({
//           id: gen_id(),
//           phieu_id: "",
//           ma_sku: ct.ma_sku,
//           ten_sp: ct.ten_sp,
//           so_luong_po: ct.so_luong,
//           so_luong_thuc: 0,
//           so_luong_dat: 0,
//           so_luong_loi: 0,
//           trang_thai_qc: "chua_kiem" as const,
...
//   const cfg = LOAI_PHIEU_CONFIG[loai];
...
//             <div>
//               <h3 className="text-sm font-black text-white">{cfg.nhan}</h3>
//               <p className="text-[10px] mt-0.5" style={{ color: "#475569" }}>
//                 {cfg.mo_ta}
//               </p>
//             </div>
...
//                       style={{ color: "#475569" }}
//                     >
//                       Không có PO nào đang chờ giao hàng
//                     </p>
//                   )}
//                   {filtered_po.map((po) => (
//                     <button
...
//                       style={{
//                         background:
//                           selected_po_id === po.id ? "#0c435425" : "#0f172a",
//                         border: `1px solid ${selected_po_id === po.id ? "#38bdf8" : "#1e293b"}`,
//                       }}
//                     >
...
//                         {po.chi_tiet.slice(0, 3).map((ct) => (
//                           <span
//                             key={ct.ma_sku}
//                             className="text-[9px] px-1.5 py-0.5 rounded font-mono"
//                             style={{ background: "#1e293b", color: "#64748b" }}
//                           >
//                             {ct.ma_sku} ×{ct.so_luong}
//                           </span>
//                         ))}
//                         {po.chi_tiet.length > 3 && (
//                           <span
//                             className="text-[9px] px-1.5 py-0.5 rounded"
//                             style={{ background: "#1e293b", color: "#475569" }}
//                           >
//                             +{po.chi_tiet.length - 3} SKU nữa
//                           </span>
//                         )}
//                       </div>
//                     </button>
...
//               {/* Khi đã chọn PO — hiện preview + hỏi ngày nhận */}
//               {selected_po && (
//                 <>
//                   {/* Preview chi tiết PO */}
//                   <div
...
//                         style={{ color: "#475569" }}
//                       >
//                         Hàng hóa trong {selected_po.ma_po}
//                       </p>
//                       <span className="text-[9px]" style={{ color: "#38bdf8" }}>
//                         {selected_po.chi_tiet.length} SKU ·{" "}
//                         {selected_po.chi_tiet.reduce(
//                           (s, c) => s + c.so_luong,
//                           0,
//                         )}{" "}
//                         cái
//                       </span>
//                     </div>
//                     {selected_po.chi_tiet.map((ct, i) => (
//                       <div
//                         key={ct.ma_sku}
//                         className="flex items-center justify-between px-4 py-2.5"
//                         style={{
//                           borderBottom:
//                             i < selected_po.chi_tiet.length - 1
//                               ? "1px solid #0f172a"
//                               : "none",
//                         }}
//                       >
//                         <div>
//                           <p className="text-xs text-white">{ct.ten_sp}</p>
//                           <code
//                             className="text-[9px]"
//                             style={{ color: "#475569" }}
//                           >
...
//                     <input
//                       value={ghi_chu_po}
//                       onChange={(e) => setGhiChuPo(e.target.value)}
