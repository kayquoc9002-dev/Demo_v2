...
import type {
  PhieuNhatHang,
  PackingItem,
} from "../../../components/Kho/data/outboundTypes";
import {
  gop_don_thanh_phieu_nhat,
  tao_packing_items,
  tinh_stats,
  gen_id,
  gen_ma_phieu_nhat,
...
import { MOCK_PHIEU_NHAT } from "../../../components/Kho/data/outboundMockData";
import {
  MOCK_SKU_RULES,
  MOCK_NODES,
} from "../../../components/Kho/data/warehouseMockData";
import { MOCK_DON_HANG } from "../../../components/BanHang/data/orderData";
...
function TabChonDon({
  ...
  on_tao_phieu: (phieu: PhieuNhatHang) => void;
}) {
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [search, setSearch] = useState("");
  const [filter_dvvc] = useState("all");

  // Chỉ lấy đơn "cho_xu_ly" (chờ xử lý)
  const don_cho_xu_ly = useMemo(
    () => MOCK_DON_HANG.filter((d) => d.trang_thai_don === "cho_xu_ly"),
    [],
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (filter_dvvc !== "all")
      list = list.filter((d) => d.don_vi_vc === filter_dvvc);
    return list;
  ...
  const preview_gop = useMemo(() => {
    ...
    return gop_don_thanh_phieu_nhat(selected_dons, MOCK_SKU_RULES, MOCK_NODES);
  }, [selected, don_cho_xu_ly]);

  const handle_tao = () => {
    const selected_dons = don_cho_xu_ly.filter((d) => selected.has(d.id));
    const danh_sach = gop_don_thanh_phieu_nhat(
      selected_dons,
      MOCK_SKU_RULES,
      MOCK_NODES,
    );
  ...
  preview_gop.forEach((d) =>
    sku_count.set(d.ma_sku, (sku_count.get(d.ma_sku) ?? 0) + d.so_luong_can),
  );
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      >
        ...
        {/* Table header */}
        <div
          className="grid px-5 py-2 text-[9px] font-black uppercase flex-shrink-0"
          style={{
        ...
        >
          {filtered.map((don) => {
            ...
            return (
              <div
                key={don.id}
                className="grid items-center px-5 py-2.5 cursor-pointer transition-colors"
                style={{
                  gridTemplateColumns: "24px 1fr 80px 80px 60px",
      ...
      <div className="flex flex-col flex-shrink-0" style={{ width: 320 }}>
        ...
        >
          ...
          <p className="text-[10px]" style={{ color: "#475569" }}>
            Hệ thống gộp theo SKU + vị trí kệ
          </p>
        </div>
          ...
          {selected.size === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-xs" style={{ color: "#475569" }}>
                Chọn đơn bên trái để xem preview
              </p>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                {[
                  ...
                  {
                    label: "Dòng nhặt",
                    val: preview_gop.length,
                    mau: "#a78bfa",
                  },
                  {
                    label: "Tổng cái",
                    val: preview_gop.reduce((s, d) => s + d.so_luong_can, 0),
                    mau: "#10b981",
                  },
                ].map(({ label, val, mau }) => (
                  <div
                  ...
                  >
                    <p className="text-base font-black" style={{ color: mau }}>
                      {val}
                    </p>
              ...
              {/* Danh sách dòng nhặt đã gộp */}
              {
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                preview_gop.map((dong, i) => (
                  <div
                    key={dong.ma_sku + dong.node_id}
                  ...
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <span
                        className="text-base font-black ml-2 flex-shrink-0"
                        style={{ color: "#38bdf8" }}
                      >
                        ×{dong.so_luong_can}
                      </span>
        ...
        >
          ...
          >
            <Plus size={15} />
            Tạo phiếu nhặt ({selected.size} đơn)
          </button>
          {selected.size > 0 && (
            <p
              ...
              style={{ color: "#475569" }}
            >
              Nhân viên sẽ đi {preview_gop.length} điểm thay vì {selected.size}{" "}
              lần
            </p>
          )}
        </div>
    ...
    <div className="flex flex-col h-full">
      ...
      >
        ...
        {da_nhat.length > 0 && (
          <>
            <p
              className="text-[9px] font-black uppercase mt-4 mb-2"
              style={{ color: "#334155" }}
            >
            ...
            {da_nhat.map((dong) => (
              <div
                key={dong.id}
                className="flex items-center gap-3 px-4 py-2.5 rounded-2xl opacity-50"
                style={{
                  background: "#06472508",
...
function TabPacking({
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Left: Danh sách đơn cần pack */}
      <div
        className="flex flex-col border-r flex-shrink-0"
        style={{ width: 300, borderColor: "#1e293b" }}
      >
        <div
        ...
        >
          {phieu.packing_items.map((item) => (
            <button
              ...
              style={{
                background:
                  selected_don === item.don_id ? "#0f172a" : "transparent",
                borderLeft:
                  selected_don === item.don_id
                    ? "2px solid #38bdf8"
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <code
                  className="text-xs font-black"
                  style={{
                    color: selected_don === item.don_id ? "#38bdf8" : "#94a3b8",
                  }}
                >
                  {item.ma_don}
                </code>
        ...
        >
          ...
          >
            Đóng gói tất cả (
            {phieu.packing_items.filter((p) => !p.da_dong_goi).length} còn lại)
          </button>
        </div>
      </div>

      {/* Right: Chi tiết đơn đang pack */}
      <div
        ...
        {!selected_don ? (
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <p className="text-sm" style={{ color: "#475569" }}>
              Chọn đơn để bắt đầu đóng gói
            </p>
          </div>
        ) : (
          (() => {
            const item = phieu.packing_items.find(
              (p) => p.don_id === selected_don,
            )!;
            return (
              <div className="space-y-4 max-w-lg">
                  ...
                  {item.san_pham.map((sp) => (
                    ...
                    >
                      <CheckCircle
                        size={14}
                        style={{
                          color: item.da_kiem_tra ? "#10b981" : "#334155",
                          flexShrink: 0,
                        }}
                      />
                      <div className="flex-1 min-w-0">
                ...
                <div className="space-y-2">
                  {!item.da_kiem_tra && (
                    <button
                      onClick={() =>
                        update_item(item.don_id, "da_kiem_tra", true)
                      }
                    ...
                    >
                      <ScanLine size={15} /> Xác nhận đã kiểm tra
                    </button>
                  )}
                  {item.da_kiem_tra && !item.da_dong_goi && (
                    <button
                      onClick={() => {
                        ...
                        const idx = phieu.packing_items.findIndex(
                          (p) => p.don_id === item.don_id,
                        );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              <Truck size={13} /> ĐVVC đã lấy hàng
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mt-3">
          ...
          <span
            ...
            style={{ color: "#10b981" }}
          >
            {phieu.packing_items.filter((p) => p.ma_van_don).length}/
            {phieu.packing_items.length} vận đơn
          </span>
        </div>
      </div>
      ...
      <div
        className="grid px-5 py-2 text-[9px] font-black uppercase flex-shrink-0"
        style={{
          gridTemplateColumns: "1fr 100px 200px 80px",
          background: "#0a1628",
          color: "#334155",
          borderBottom: "1px solid #1e293b",
        }}
      ...
      >
        {
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          phieu.packing_items.map((item, i) => (
            <div
              key={item.don_id}
              ...
              style={{
                gridTemplateColumns: "1fr 100px 200px 80px",
                borderBottom: "1px solid #0f172a",
                background: item.ma_van_don ? "#06472508" : "transparent",
              }}
            >
              {/* Đơn */}
              <div>
                ...
                {item.ma_van_don ? (
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVanDonInput((prev) => ({
                        ...prev,
                        [item.don_id]: e.target.value,
                      }))
                    }
                    onKeyDown={(e) =>
                      e.key === "Enter" && cap_nhat_van_don(item.don_id)
                    }
              ...
              <div className="flex items-center justify-center gap-1">
                {!item.ma_van_don ? (
                  <button
                    onClick={() => cap_nhat_van_don(item.don_id)}
...
export default function OutboundManager() {
  const [phieu_list, setPhieuList] = useState<PhieuNhatHang[]>(MOCK_PHIEU_NHAT);
  ...
  const handle_tao_phieu = (phieu: PhieuNhatHang) => {
    ...
    show_toast(`Đã tạo ${phieu.ma_phieu} — gộp ${phieu.don_ids.length} đơn`);
  };

  const handle_update = (phieu: PhieuNhatHang) => {
    setPhieuList((prev) => prev.map((p) => (p.id === phieu.id ? phieu : p)));
    // Tự động chuyển tab theo trạng thái
    if (phieu.trang_thai === "nhat_xong") {
      setActiveTab("packing");
      show_toast("Nhặt xong! Chuyển sang đóng gói.");
    } else if (phieu.trang_thai === "cho_shipping") {
      setActiveTab("shipping");
      show_toast("Đóng gói xong! Nhập mã vận đơn.");
    } else if (phieu.trang_thai === "hoan_thanh") {
  ...
  const stats_all = useMemo(
    () => ({
      ...
      cho_shipping: phieu_list.filter((p) => p.trang_thai === "cho_shipping")
        .length,
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (tt === "cho_nhat" || tt === "dang_nhat")
                    setActiveTab("nhat_hang");
                  else if (tt === "nhat_xong" || tt === "dang_packing")
                    setActiveTab("packing");
                  else if (tt === "cho_shipping" || tt === "hoan_thanh")
                    setActiveTab("shipping");
                }}
                className="w-full text-left px-4 py-3 transition-all"
                style={{
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${stats.pct_nhat}%`,
                        background:
                          stats.pct_nhat === 100 ? "#10b981" : "#38bdf8",
                      }}
                    />
                  </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          {TABS.map(({ id, label, icon: Icon }) => {
            const is_avail =
              active_tab === "chon_don" && !selected_phieu
                ? id === "chon_don"
            ...
            return (
              ...
              >
                ...
                {id === "nhat_hang" &&
                  selected_phieu &&
                  tinh_stats(selected_phieu).con_lai > 0 && (
                    <span
